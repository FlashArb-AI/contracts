...
contract TestArbitrage is IFlashLoanRecipient, ReentrancyGuard, Ownable, Pausable {
    ...
    /// @notice Restricts access to authorized traders or owner
    /// @dev Prevents unauthorized trade execution
    modifier onlyAuthorized() {
        require(authorizedTraders[msg.sender] || msg.sender == owner(), "TestArbitrage: Not authorized to trade");
        _;
    }
    ...
    modifier validTradeParams(TradeParams memory params) {
        ...
        require(
            params.routerPath[0] != address(0) && params.routerPath[1] != address(0),
            "TestArbitrage: Invalid router addresses"
        );
        require(
            params.tokenPath[0] != address(0) && params.tokenPath[1] != address(0),
            "TestArbitrage: Invalid token addresses"
        );
        require(params.fee > 0 && params.fee <= 10000, "TestArbitrage: Invalid fee tier");
        require(
            params.minProfitBps >= MIN_PROFIT_BPS && params.minProfitBps <= MAX_BPS,
            "TestArbitrage: Invalid profit threshold"
        );
